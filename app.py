...
import os
import pickle
from utils.resume_parser import ( extract_text_from_pdf, extract_text_from_docx, extract_name_and_email )
...
def upload_resume():
    ...
    for resume_file in resume_files:
        ...
        if resume_file and (
            resume_file.filename.endswith('.pdf') or
            resume_file.filename.endswith('.docx')
        ):
            ...
            name, email = extract_name_and_email(
                resume_text
            )
    ...
    if all_results:
        ...
        all_results[0]['top_candidate'] = True
...
def admin_dashboard():
    ...
    min_exp = request.args.get(
        'min_exp',
        type=int
    )

    skill_filter = request.args.get(
        'skills',
        ''
    )

    entries = ResumeEntry.query.order_by(
        ResumeEntry.match_score.desc()
    ).all()
    ...
    for entry in entries:
        ...
        if (
            ...
            entry.experience_years is not None
        ):
        ...
        if skill_filter:
            ...
            required_skills = [
                s.strip().lower()
                for s in skill_filter.split(',')
            ]

            if entry.skills:

                if not all(
                    skill in entry.skills.lower()
...
def send_to_ta():
    ...
    if selected_ids:
        ...
        selected_profiles = ResumeEntry.query.filter(
            ResumeEntry.id.in_(selected_ids)
        ).all()

        return render_template(
            'ta_confirmation.html',
